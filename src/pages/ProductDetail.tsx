...
//         const allProductsResponse = await axios.get('/api/sarees');
//         const allProducts = allProductsResponse.data;

//         const related = allProducts.filter((p: SareeData) =>
//           p.id !== productData.id &&
...
//   const relatedFabricProducts = relatedProducts.filter(
//     p => p.fabric === product.fabric
//   ).slice(0, 4);

//   const relatedCategoryProducts = relatedProducts.filter(
//     p => p.category === product.category && p.fabric !== product.fabric
//   ).slice(0, 4);
...
//             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
//               {relatedCategoryProducts.map((rp: SareeData) => (
//                 <div
//                   key={rp.id}
//                   onClick={() => navigate(`/product/${rp.id}`)}
//                   className="border rounded-lg overflow-hidden hover:shadow-lg transition cursor-pointer"
...
// ✅ Axios instance defined inside the file
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/api/sarees/${id}`);
        const productData = response.data;
        setProduct(productData);
        setSelectedImage('');
        setQuantity(1);
        ...
        const allProductsResponse = await api.get('/api/sarees');
        const allProducts = allProductsResponse.data;
  ...
  const addToCart = async () => {
    try {
      ...
      const response = await api.post(`/api/cart/${sessionId}/add`, {
        sareeId: product?.id,
        quantity: quantity,
      });
  ...
  // Images & Related Product Filtering
  const allImages = product.images?.length ? product.images : [product.image];
  const mainImage = selectedImage || allImages[0];
  const thumbnails = [mainImage, ...allImages.filter(img => img !== mainImage)];
            ...
            <div className="space-y-4">
              ...
              {product.addin === 'sarees' && (
                <div className="flex space-x-4">
                  ...
                  >
                    <ShoppingCart size={20} />
                    <span>{!product.inStock ? 'OUT OF STOCK' : 'ADD TO BAG'}</span>
                  </button>
                  <button className="p-3 border border-gray-300 hover:bg-gray-50 transition-colors duration-200">
                    <Heart size={20} />
                  </button>
